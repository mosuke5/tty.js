...
Server.prototype.handleConnection = function(socket) {
  ...
  //mosuke5 add 
  socket.on('tmp_id', function(data) {
    socket.set('tmp_id', data);
  });
};
...
Session.prototype.handleCreate = function(cols, rows, func) {
  ...
  //mosuke5 add
  socket.get('tmp_id', function(err,name){
    
    var new_shellArgs = [];

    for (var i = 0 ; i < shellArgs.length ; i++){
      new_shellArgs.push(shellArgs[i]);
    }

    //tmp_idをpush
    new_shellArgs.push(name);

    term = pty.fork(shell, new_shellArgs, {
      name: conf.termName,
      cols: cols,
      rows: rows,
